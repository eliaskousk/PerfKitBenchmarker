...
# PACKAGE_NAME = 'fio_charon_ssp' # Should be same as related BENCHMARK_NAME
# PACKAGE_DATA = ['opencsw_pkg_admin.txt']
# OPENCSW_PKG_ADMIN = 'opencsw_pkg_admin.txt'
...
def GetFioExec():
  ...
  return '{path}'.format(path=FIO_PATH)
...
def _Install(vm):
  ...
  # scp_cmd = 'scp %s -i ~/.ssh/ssp_solaris_rsa %s root@%s:/%s'\
  #           % (ssh_options,
  #              posixpath.join(vm_util.VM_TMP_DIR, OPENCSW_PKG_ADMIN),
  #              vm.secondary_nic.private_ip_address,
  #              OPENCSW_PKG_ADMIN)
  #
  # vm.RemoteCommand(scp_cmd)

  # ssh_prefix = 'ssh %s root@%s' % (ssh_options, vm.secondary_nic.private_ip_address)

  # vm.RemoteCommand("%s 'yes all | pkgadd -n -a /%s -d http://get.opencsw.org/now'"
  #                  % (ssh_prefix, OPENCSW_PKG_ADMIN))
  # vm.RemoteCommand("%s '/opt/csw/bin/pkgutil -U'"
  #                  % ssh_prefix)
  # vm.RemoteCommand("%s '/opt/csw/bin/pkgutil -y -i fio'"
  #                  % ssh_prefix)
  ...
  for job in fio_json_result['jobs']:
    ...
    for mode in io_modes:
      if job[mode]['io_bytes']:
        ...
        clat_key = 'clat' if 'clat' == job[mode] else 'clat_ns'
