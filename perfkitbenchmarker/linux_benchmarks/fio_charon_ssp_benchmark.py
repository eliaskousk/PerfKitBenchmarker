...
from perfkitbenchmarker.linux_packages import fio_charon_ssp
import six
...
SOLARIS_FIO_DIR = '/export/home/fio/'
SOLARIS_JOB_FILE_PATH = posixpath.join(SOLARIS_FIO_DIR, 'fio.job')
...
flags.DEFINE_list('fio_charon_ssp_generate_scenarios', [],
                  ...
                  '--fio_jobfile.')
flags.DEFINE_enum('fio_charon_ssp_target_mode', AGAINST_FILE_WITHOUT_FILL_MODE,
                  ...
                  'to prefill.')
flags.DEFINE_string('fio_charon_ssp_fill_size', '100%',
...
flag_util.DEFINE_integerlist('fio_charon_ssp_io_depths', flag_util.IntegerList([1]),
...
flag_util.DEFINE_integerlist('fio_charon_ssp_num_jobs', flag_util.IntegerList([1]),
                             'Number of concurrent fio jobs to run.',
                             on_nonincreasing=flag_util.IntegerListParser.WARN,
                             module_name=__name__)
flags.DEFINE_integer('fio_charon_ssp_working_set_size', None,
                     ...
                     lower_bound=0)
flag_util.DEFINE_units('fio_charon_ssp_blocksize', None,
...
flags.DEFINE_integer('fio_charon_ssp_runtime', 600,
                     'The number of seconds to run each fio job for.',
                     lower_bound=1)
flags.DEFINE_list('fio_charon_ssp_parameters', ['randrepeat=0'],
                  'Parameters to apply to all PKB generated fio jobs. Each '
                  'member of the list should be of the form "param=value".')
flags.DEFINE_boolean('fio_charon_ssp_lat_log', False,
                     'Whether to collect a latency log of the fio jobs.')
flags.DEFINE_boolean('fio_charon_ssp_bw_log', False,
                     'Whether to collect a bandwidth log of the fio jobs.')
flags.DEFINE_boolean('fio_charon_ssp_iops_log', False,
                     'Whether to collect an IOPS log of the fio jobs.')
flags.DEFINE_integer('fio_charon_ssp_log_avg_msec', 1000,
                     ...
                     lower_bound=0)
flags.DEFINE_boolean('fio_charon_ssp_hist_log', False,
                     'Whether to collect clat histogram.')
flags.DEFINE_integer('fio_charon_ssp_log_hist_msec', 1000,
...
flags.DEFINE_boolean(  # TODO(user): Add support for simultaneous read.
    'fio_charon_ssp_write_against_multiple_clients', False,
    'Whether to run fio against multiple nfs. Only applicable '
    'when running fio against network mounts and rw=write.')
flags.DEFINE_integer('fio_charon_ssp_command_timeout_sec', None,
                     'Timeout for fio commands in seconds.')

FLAGS_IGNORED_FOR_CUSTOM_JOBFILE = {
    'fio_charon_ssp_generate_scenarios', 'fio_charon_ssp_io_depths', 'fio_charon_ssp_runtime',
    'fio_charon_ssp_blocksize', 'fio_charon_ssp_num_jobs', 'fio_charon_ssp_parameters'}

BENCHMARK_NAME = 'fio_charon_ssp'

BENCHMARK_CONFIG = """
fio_charon_ssp:
  description: Runs fio on Charon SSP SPARC emulator (sequential, random, read & write modes)
  vm_groups:
    default:
...
BENCHMARK_DATA = {
    'fio_solaris.job':
      '0ec119dd2d602fe2bd8827ffe65e1604dd5cf9e524596c5f7c17201b05f6ccc4'
}
...
def AgainstDevice():
  ...
  """
  return FLAGS.fio_charon_ssp_target_mode in AGAINST_DEVICE_MODES
...
def FillTarget():
  ...
  """
  return FLAGS.fio_charon_ssp_target_mode in FILL_TARGET_MODES
...
def WarnOnBadFlags():
  ...
  if FLAGS.fio_charon_ssp_jobfile:
  ...
  if (FLAGS.fio_charon_ssp_jobfile is None and
      FLAGS.fio_charon_ssp_generate_scenarios and
      not FLAGS.fio_charon_ssp_working_set_size and
      not AgainstDevice()):
...
def GetConfig(user_config):
  ...
  if FLAGS.fio_charon_ssp_target_mode != AGAINST_FILE_WITHOUT_FILL_MODE:
...
def GetLogFlags(log_file_base):
  """Gets fio log files."""
  collect_logs = FLAGS.fio_charon_ssp_lat_log or FLAGS.fio_charon_ssp_bw_log or FLAGS.fio_charon_ssp_iops_log
  fio_log_flags = [(FLAGS.fio_charon_ssp_lat_log, '--write_lat_log=%(filename)s',),
                   (FLAGS.fio_charon_ssp_bw_log, '--write_bw_log=%(filename)s',),
                   (FLAGS.fio_charon_ssp_iops_log, '--write_iops_log=%(filename)s',),
                   (FLAGS.fio_charon_ssp_hist_log, '--write_hist_log=%(filename)s',),
                   (collect_logs, '--log_avg_msec=%(interval)d',)]
  fio_command_flags = ' '.join([flag for given, flag in fio_log_flags if given])
  if FLAGS.fio_charon_ssp_hist_log:
    fio_command_flags = ' '.join([
        fio_command_flags, '--log_hist_msec=%(hist_interval)d'])

  return fio_command_flags % {'filename': log_file_base,
                              'interval': FLAGS.fio_charon_ssp_log_avg_msec,
                              'hist_interval': FLAGS.fio_charon_ssp_log_hist_msec}
...
def Prepare(benchmark_spec):
  exec_path = fio_charon_ssp.GetFioExec()
...
def PrepareWithExec(vm, exec_path):
  ...
  vm.Install('fio_charon_ssp')


def Run(benchmark_spec):
  """Spawn fio on vm(s) and gather results."""

  fio_exe = fio_charon_ssp.GetFioExec()
  default_job_file_contents = GetFileAsString(data.ResourcePath('fio_solaris.job'))
  vms = benchmark_spec.vms
  samples = []
  ...
  for item in samples:
    item.metadata['fio_target_mode'] = FLAGS.fio_charon_ssp_target_mode
    item.metadata['fio_fill_size'] = FLAGS.fio_charon_ssp_fill_size
...
def RunWithExec(vm, exec_path, remote_job_file_path, job_file_contents):
  ...
  job_file_string = GetOrGenerateJobFileString(
      FLAGS.fio_charon_ssp_jobfile,
      FLAGS.fio_charon_ssp_generate_scenarios,
      AgainstDevice(),
      None,
      FLAGS.fio_charon_ssp_io_depths,
      FLAGS.fio_charon_ssp_num_jobs,
      FLAGS.fio_charon_ssp_working_set_size,
      FLAGS.fio_charon_ssp_blocksize,
      FLAGS.fio_charon_ssp_runtime,
      FLAGS.fio_charon_ssp_parameters,
      job_file_contents)
  ...
  vm.PushFile(job_file_path, remote_job_file_path,
              nested_remote_path=SOLARIS_JOB_FILE_PATH)

  fio_command = '%s --output-format=json --directory=%s %s'\
                % (exec_path, SOLARIS_FIO_DIR, SOLARIS_JOB_FILE_PATH)

  logging.info('FIO Results:')

  start_time = time.time()
  stdout, _ = vm.RobustRemoteCommand(
      fio_command, nested=True, should_log=True,
      timeout=FLAGS.fio_charon_ssp_command_timeout_sec)
  end_time = time.time()

  samples = fio_charon_ssp.ParseResults(job_file_string, json.loads(stdout))
...
def Cleanup(benchmark_spec):
  ...
  logging.info('Shutting down Charon SSP on %s', vm)
